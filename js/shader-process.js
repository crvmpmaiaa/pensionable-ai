/* WebGL plasma shader background for the process section */
(function () {
  ...
  const fsSource = `
    ...
    float drawCrispLineFn(float pos, float halfWidth, float t) {
  ...
  function init() {
    // Shader disabled — no target section
    return;
    ...
    // Ensure content sits above canvas without disturbing layout
    Array.from(section.children).forEach(el => {
      if (el !== canvas) el.style.zIndex = '2';
    });
